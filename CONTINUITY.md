...
- tracker/application/use_cases/backfill_trend_history.py
- tracker/application/use_cases/run_trend_engine.py
- tracker/infrastructure/export/xlsx_exporter.py
- tracker/infrastructure/storage/sqlite_state_repository.py
...
- 2026-05-24: `git worktree list` showed only `/Users/roman/Documents/Development/hedge_funds_tracker`; `git status --short` was clean on `main`.
...
- 2026-05-24: Temporary e2e smoke passed: tracker backfill CLI restored missing option rows, `python -m tracker --show-trends-only` and `scripts/show_trends.py` rendered option sections, and option-only XLSX export produced `Call Option Trends`.
