...
from signals.domain.models import DiffResult, Position
...
def _primary_metric(position: Position) -> int | None:
...
def diff_positions(previous: list[Position], current: list[Position]) -> DiffResult:
...
def summarize_position(position: Position) -> str:
...
def build_diff_message(result: DiffResult, *, max_lines: int = 15) -> str:
    ...
    def add_section(title: str, positions: list[Position]) -> None:
        if not positions:
            return
